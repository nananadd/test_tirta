...
<div class="row mb-4 align-items-center">
    ...
    <div class="col-md-8 text-md-end mt-3 mt-md-0 pe-3">
        ...
        <a href="{{ route('table_a.export.excel') }}" class="btn btn-outline-success btn-custom me-2 btn-preview-excel"><i class="fa-solid fa-file-excel"></i> Excel</a>
        <a href="{{ route('table_a.export.pdf') }}" target="_blank" class="btn btn-outline-danger btn-custom"><i class="fa-solid fa-file-pdf"></i> PDF</a>
    </div>
</div>
    ...
    <div class="card-body p-0">
        ...
        <div class="table-responsive p-4">
            <table class="table table-hover align-middle m-0">
                ...
                <tbody>
                    @forelse($table_a_data as $index => $item)
                    <tr>
                        <td class="text-center text-muted fw-bold">{{ $index + 1 }}</td>
                        <td><span class="badge bg-primary bg-opacity-10 text-primary border border-primary px-3 py-2" style="border-radius: 8px;">{{ $item->kode_toko_baru }}</span></td>
                        <td class="fw-bold text-secondary">{{ $item->kode_toko_lama ?? 'Tidak Ada' }}</td>
                        <td class="text-center">
                            <div class="d-flex justify-content-center gap-2">
                                <button class="btn btn-sm btn-warning text-dark fw-bold" style="border-radius: 8px;" data-bs-toggle="modal" data-bs-target="#editModal{{ $item->kode_toko_baru }}"><i class="fa-solid fa-pen-to-square"></i></button>
                                <form action="{{ route('table_a.destroy', $item->kode_toko_baru) }}" method="POST">
                                    @csrf @method('DELETE')
                    ...
                    @empty
                    <tr><td colspan="4" class="text-center py-5 text-muted"><i class="fa-regular fa-folder-open fa-3x mb-3 d-block"></i>Belum ada data toko.</td></tr>
                    @endforelse
                </tbody>
            </table>
...
@foreach($table_a_data as $item)
<div class="modal fade" id="editModal{{ $item->kode_toko_baru }}" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 shadow" style="border-radius: 16px;">
            <div class="modal-header border-bottom-0 pb-0">
                <h5 class="fw-bold text-dark"><i class="fa-solid fa-pen-to-square text-warning me-2"></i>Edit Data Toko</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <form action="{{ route('table_a.update', $item->kode_toko_baru) }}" method="POST">
                    @csrf @method('PUT')
                    <div class="mb-3">
                        <label class="form-label small fw-bold text-muted">Kode Toko Baru (Primary Key)</label>
                        <input type="text" class="form-control bg-light text-muted" value="{{ $item->kode_toko_baru }}" readonly>
                    </div>
                    <div class="mb-4">
                        <label class="form-label small fw-bold text-muted">Kode Toko Lama</label>
                        <input type="number" class="form-control" name="kode_toko_lama" value="{{ $item->kode_toko_lama }}">
                    </div>
        ...
        <div class="modal-content border-0 shadow" style="border-radius: 16px;">
            ...
            <div class="modal-body">
                <form action="{{ route('table_a.store') }}" method="POST">
                    @csrf
                    <div class="mb-3">
                        <label class="form-label small fw-bold text-muted">Kode Toko Baru</label>
                        <input type="number" class="form-control" name="kode_toko_baru" placeholder="Kode Toko Baru" required>
                    </div>
                    <div class="mb-4">
                        <label class="form-label small fw-bold text-muted">Kode Toko Lama (Opsional)</label>
                        <input type="number" class="form-control" name="kode_toko_lama" placeholder="Kode Toko Lama">
                    </div>
