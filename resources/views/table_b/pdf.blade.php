...
<html>
<head>
    <title>Laporan Data Transaksi (Tabel B)</title>
    <style>
        ...
        .badge { background-color: #e8f8f5; color: #117a65; padding: 5px 10px; border-radius: 4px; font-weight: bold; font-size: 12px; display: inline-block; border: 1px solid #d1f2eb; }
...
<body>
    ...
    <div class="header">
        <h2>Laporan Data Transaksi Toko (Tabel B)</h2>
    ...
    <table>
        <thead>
            <tr>
                ...
                <th style="width: 45%;">Kode Toko</th>
                <th>Nominal Transaksi</th>
            </tr>
        </thead>
        <tbody>
            @php $no = 1; @endphp
            @forelse($table_b_data as $item)
                <tr>
                    <td style="text-align: center; color: #7f8c8d;">{{ $no++ }}</td>
                    <td><span class="badge">{{ $item->kode_toko }}</span></td>
                    <td class="text-success">Rp {{ number_format($item->nominal_transaksi, 2, ',', '.') }}</td>
                </tr>
            @empty
                <tr>
                    <td colspan="3" style="text-align: center; color: #95a5a6; font-style: italic; padding: 20px;">Tidak ada data transaksi di database.</td>
                </tr>
            @endforelse
        </tbody>
    </table>
