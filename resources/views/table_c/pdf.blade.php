...
<html>
<head>
    <title>Laporan Data Area (Tabel C)</title>
    <style>
...
<body>
    ...
    <div class="header">
        <h2>Laporan Wilayah Kerja Area (Tabel C)</h2>
    ...
    <table>
        ...
        <tbody>
            @php $no = 1; @endphp
            @forelse($table_c_data as $item)
                <tr>
                    <td style="text-align: center; color: #7f8c8d;">{{ $no++ }}</td>
                    <td><span class="badge">{{ $item->kode_toko }}</span></td>
                    <td class="text-info">Area {{ $item->area_sales }}</td>
                </tr>
            @empty
                <tr>
                    <td colspan="3" style="text-align: center; color: #95a5a6; font-style: italic; padding: 20px;">Tidak ada data area di database.</td>
                </tr>
            @endforelse
        </tbody>
    </table>
